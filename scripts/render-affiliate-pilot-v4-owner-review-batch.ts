...
const cards = reviewJobs
  .map(
    (job) => `<article class="card" data-number="${job.reviewNumber}">
      ...
      <textarea rows="2" placeholder="Reason required for Deny; optional note for Approve"></textarea>
    </article>`
  )
  .join("\n");
...
function render(){let count=0;cards.forEach(card=>{const n=card.dataset.number;const entry=state[n]||{};const missingNote=entry.decision==='DENY'&&!(entry.note||'').trim();card.classList.toggle('approve',entry.decision==='APPROVE');card.classList.toggle('deny',entry.decision==='DENY');card.classList.toggle('missing-note',missingNote);card.querySelectorAll('[data-decision]').forEach(b=>b.classList.toggle('active',b.dataset.decision===entry.decision));const note=card.querySelector('textarea');if(document.activeElement!==note)note.value=entry.note||'';note.setAttribute('aria-invalid',missingNote?'true':'false');if(entry.decision&&!missingNote)count++});document.querySelector('#progress').textContent=count+' / '+cards.length+' complete'}
cards.forEach(card=>{const n=card.dataset.number;card.querySelectorAll('[data-decision]').forEach(b=>b.onclick=()=>{state[n]={...(state[n]||{}),decision:b.dataset.decision};save();if(b.dataset.decision==='DENY'&&!(state[n].note||'').trim())card.querySelector('textarea').focus()});card.querySelector('textarea').oninput=e=>{state[n]={...(state[n]||{}),note:e.target.value};save()}});
function rows(){return cards.map(card=>{const n=card.dataset.number;const entry=state[n]||{};return {reviewNumber:Number(n),sceneId:${JSON.stringify(reviewJobs.map((job) => job.sceneId))}[Number(n)-1],decision:entry.decision||'UNDECIDED',note:(entry.note||'').trim()}})}
function validateDenials(){const missing=rows().filter(r=>r.decision==='DENY'&&!r.note);if(!missing.length)return true;const card=cards.find(c=>Number(c.dataset.number)===missing[0].reviewNumber);card.scrollIntoView({behavior:'smooth',block:'center'});card.querySelector('textarea').focus();window.alert('Add a reason for every denied image before copying or downloading decisions.');return false}
document.querySelector('#copy').onclick=async()=>{if(!validateDenials())return;const text=rows().map(r=>String(r.reviewNumber).padStart(3,'0')+' '+r.decision+(r.note?' — '+r.note:'')).join('\\n');await navigator.clipboard.writeText(text);document.querySelector('#copy').textContent='Copied';setTimeout(()=>document.querySelector('#copy').textContent='Copy decision list',1200)};
document.querySelector('#download').onclick=()=>{if(!validateDenials())return;const blob=new Blob([JSON.stringify({batchId:${JSON.stringify(batchId)},decisions:rows()},null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=${JSON.stringify(`${batchId}-decisions.json`)};a.click();URL.revokeObjectURL(a.href)};
