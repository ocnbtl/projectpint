...
function render(){let count=0;cards.forEach(card=>{const n=card.dataset.number;const entry=state[n]||{};const missingNote=entry.decision==='DENY'&&!entry.note;card.classList.toggle('approve',entry.decision==='APPROVE');card.classList.toggle('deny',entry.decision==='DENY');card.classList.toggle('missing-note',missingNote);card.querySelectorAll('[data-decision]').forEach(b=>b.classList.toggle('active',b.dataset.decision===entry.decision));const note=card.querySelector('textarea');note.value=entry.note||'';note.setAttribute('aria-invalid',missingNote?'true':'false');if(entry.decision&&!missingNote)count++});document.querySelector('#progress').textContent=count+' / '+cards.length+' complete'}
cards.forEach(card=>{const n=card.dataset.number;card.querySelectorAll('[data-decision]').forEach(b=>b.onclick=()=>{state[n]={...(state[n]||{}),decision:b.dataset.decision};save();if(b.dataset.decision==='DENY'&&!state[n].note)card.querySelector('textarea').focus()});card.querySelector('textarea').oninput=e=>{state[n]={...(state[n]||{}),note:e.target.value.trim()};save()}});
