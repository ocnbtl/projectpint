...
export const affiliatePilotV4RealismReset = {
  contractVersion: "affiliate-pilot-real-bathroom-v4.38",
  ...
  nonTextileRootCauseRevision:
    "The first OXO hard-product scene proved that the room-first reset works for non-textiles, but both allowed direct attempts failed small identity or secondary-object gates. Attempt one produced a strong candid room while omitting the canonical lower-right oval mark and adding a competing pump bottle. Attempt two produced another strong room but collapsed the short forward spout into a cap, misplaced the mark, and added labeled pump bottles and pseudo-text despite the direct prompt. Regenerating a third whole room would discard passing camera, light, architecture, material, and human-trace evidence while leaving the small-region failure mechanism unchanged. V4.25 strengthens the secondary-object invariant and adds a provider-native same-scene near-pass correction lane for non-textiles. Its first use preserved the passing room, removed every label-bearing ambiguity, and restored the spout and mark, but mirrored the handed pump spout relative to the visible canonical front. V4.26 raises the correction allowance from one to two only for this localized lane and requires an explicit camera-projected handedness directive before the second attempt. The final Scene 01 correction passed, and Scene 02 passed after an ordinary direct composition retry. Scene 03 then mirrored the top-view spout on its first direct attempt, proving viewer-space handedness belongs in each OXO placement prompt rather than only the correction lane. Preflight also caught a global human-trace recipe that requested a second pump bottle for Scene 05, directly contradicting the no-second-product-category rule. V4.27 encodes the reviewed viewer-space spout projection for all five countertop-dispenser slots and replaces that conflicting trace with a plain grooming brush. The first complete Modern Marble set then passed individual room, product, and physical review but failed its contact-sheet audit because all five scenes literalized the style as nearly identical white-gray veined counters, floors, and wet-zone walls. The same set also produced packaged toiletry defects encouraged by an optional style-vocabulary line that contradicted the no-bottle and no-package gate. V4.28 removes optional movable style props from styled prompts, makes the assigned human trace the only movable-object source, adds five deterministic set-level fixed-surface expression lanes, limits any obvious signature color or material to one major surface per scene, and hard-rejects repeated dominant surface and palette combinations across a five-scene set. Both allowed V4.28 retries for the first Modern Marble slot still generated veined stone on the assigned counter and on the floor, proving that abstract ordinary or non-signature wording was not operationally specific enough. V4.29 adds five Modern Marble slot overrides with explicit allowed signature surfaces, positive ordinary-finish alternatives, and zero-vein negatives for every other major surface. Scene 03 then ignored its zero-vein counter contract, repeated white-gray stone on the counter and wet zone, duplicated a toilet, and added an unassigned container. The generic style description still began with a request for veined stone and therefore competed with the later slot contract. V4.30 makes the slot lane authoritative over the style name and generic description, removes implied marble permission from the Modern Marble atmosphere line, turns slot 03 into a no-visible-marble hardware-and-period-detail lane, and requires an explicit per-surface audit before output. Before the next set, preflight found the same structural contradiction in other legacy descriptions, including Spa Greenery explicitly requesting lush plants and steam while the movable-object ceiling forbids plants and atmospheric staging. V4.31 suppresses every legacy style description in styled prompts, treats all style names as atmosphere labels only, and makes the fixed-surface lane plus assigned Human trace the exclusive sources of style expression and movable objects. The first V4.31 Spa Greenery scene then chose both a step stool and hair tie from an or-list and invented several more fabrics, a rug, open storage, and drawer contents to satisfy the global request for three signs of use; it also drew stacked ordinary receptacles instead of one bathroom GFCI. V4.32 makes or-lists exclusive, makes and-lists exact, requires all extra occupancy evidence to be fixed wear or age rather than objects, removes the remaining optional-decor allowance, and hard-rejects any visible non-GFCI or duplicated bathroom receptacle. The next V4.32 scene reduced the failure to one extra or-list object and produced a correct GFCI, but it still repeated signature sage green across both the counter and main walls despite a counter-only generic lane. V4.33 gives Spa Greenery five explicit allowed and forbidden color-surface maps, including neutral positive finishes and zero-signature-green requirements for every nonassigned major surface. V4.33 then fixed the surfaces but the provider combined three alternatives from the same Human trace despite the exclusivity sentence. V4.34 removes every or-list and underspecified grouping from the ten reusable Human trace recipes; each manifest job now receives concrete, countable objects with no alternatives left for the provider to combine. Both allowed V4.34 countertop-dispenser slot-02 calls still collapsed into clean, near-centered vanity close-ups, and the second also mirrored the required spout projection. V4.35 replaces that product-role-and-slot composition with a broader room contract requiring two non-vanity zones, a vanity below half-frame, an outer-third product position, and 8–14 percent product height. Before the next manifest set, preflight found Brass & Terrazzo still depended on the same abstract signature-material wording that had already failed Modern Marble and Spa Greenery. V4.36 adds five explicit Brass & Terrazzo surface maps: counter-only terrazzo, wet-zone-only terrazzo, floor-only terrazzo, brass-only owner update with zero terrazzo, and alternate-palette joinery with zero terrazzo and no repeated brass.",
  nonTextileLatestRootCauseRevision:
    "Both allowed V4.36 Brass & Terrazzo Scene 01 calls obeyed the counter-only terrazzo map but repeated champagne brass across multiple fixture families and collapsed the wide room slot into a counter-dominant vanity crop. V4.37 assigns one exact brass family per slot, lists the required non-brass finish for every other hardware class, removes generic profile language that encouraged repeated warm metal, and adds a countertop-dispenser slot-01 broad-room contract requiring two non-vanity zones, a vanity below half-frame, an outer-third product position, and 8–14 percent product height. Both allowed V4.37 Brass & Terrazzo Scene 03 calls then converted a requested two-inch-open drawer into a broadly open drawer or tilt-out compartment with exposed storage. V4.38 retires that fragile open-storage geometry across the manifest. The replacement trace keeps every cabinet fully closed, uses one slightly off-level fixed cabinet pull plus one visibly angled folded washcloth, and forbids every other movable object.",
} as const;
...
export const affiliatePilotV4ExecutionPolicy = {
  ...
  nonTextileNearPassCorrectionProviderAttemptBudget: 2,
  ...
  nonTextileNearPassCorrectionRequiresCameraProjectedHandednessDirective: true,
  ...
  countertopDispenserViewerSpaceHandednessBySlotRequired: true,
  humanTraceMayNotConflictWithFeaturedProductCategoryInvariant: true,
...
export const affiliatePilotV4HumanTraceRecipes = [
  ...
  "One robe carries one natural compressed fold, while a pair of slippers point in different directions but remain fully supported.",
  "A small damp patch darkens part of one towel and a plain soap bar shows one softened used edge.",
  "A cabinet door is almost, but not completely, closed and one plain grooming brush rests where a person actually left it.",
  "One plain ceramic cup sits near the sink with a faint water ring; no other movable object is present.",
  "One child's step stool appears as the single ordinary nondecorative interruption; the rest of the room contains no movable object.",
  "One towel has been refolded imperfectly and one plain hair tie rests nearby; no container is present.",
...
export const affiliatePilotV4StyleSetExpressionLanes = [
  ...
  "Express the style as one achievable owner update on a single large surface. Keep every other fixed finish visibly older, ordinary, and different from slots 1 through 3.",
...
export const affiliatePilotV4StyleSpecificSetExpressionOverrides = {
  "modern-marble": [
    ...
    "HARDWARE-AND-PERIOD-DETAIL LANE. Visible marble, marble-look, stone veining, and white-gray Carrara are forbidden everywhere in this scene. Express the style only through restrained matte-black or aged-metal hardware, a cool cream and charcoal palette, clean-lined joinery, and one retained period detail. REQUIRED: SURFACE MAP: vanity counter = solid unpatterned zero-vein finish; floor = plain solid-color ceramic, porcelain, or resilient finish with zero veins; every wet-zone wall = plain solid-color tile or plaster with zero veins; main wall = ordinary painted plaster or drywall.",
    "OWNER-UPDATE VERTICAL LANE. Use one achievable marble-look backsplash or short vertical wainscot as the only veined surface. REQUIRED: the vanity counter, floor, shower wall, tub surround, and main wall are ordinary solid zero-vein finishes with retained older character.",
    "ALTERNATE-PALETTE JOINERY LANE. White-gray Carrara is forbidden. Use a furniture-like vanity and, if needed, one small muted green, brown, cream, or charcoal stone sill or backsplash as the only veined surface. REQUIRED: counter, floor, and wet-zone walls otherwise remain plain solid zero-vein finishes."
  ],
  "spa-greenery": [
    "COUNTER-OR-BACKSPLASH GREEN LANE. Signature mineral, eucalyptus, sage, or moss green is allowed only on one vanity counter or its short backsplash. REQUIRED: floor = plain cream, warm gray, or natural wood-look with zero green; wet-zone wall = plain white, cream, or warm gray with zero green; main wall = ordinary warm white, cream, or light neutral gray paint with zero green. Add no plant.",
    "WET-ZONE GREEN LANE. Signature mineral, eucalyptus, sage, or moss green is allowed only on one shower, tub, or wainscot field. REQUIRED: vanity top = plain cream, warm white, gray, or natural wood with zero green; floor = plain neutral with zero green; main wall = ordinary warm white or cream paint with zero green. Add no plant.",
    "FLOOR-TRIM-OR-HARDWARE GREEN LANE. Signature green is allowed only on one floor, trim, or fixed-hardware decision. REQUIRED: vanity top = plain warm white, cream, or wood with zero green; wet-zone wall = plain neutral with zero green; main wall = ordinary warm white, cream, or light gray paint with zero green. Add no plant.",
    "OWNER-UPDATE SINGLE GREEN SURFACE LANE. Choose exactly one green-painted vanity, green backsplash, or green wainscot as the only signature-green major surface. REQUIRED: every other counter, floor, wet-zone wall, and main wall uses an older ordinary warm-white, cream, gray, or natural-wood finish with zero green. Add no plant.",
    "ALTERNATE-PALETTE JOINERY LANE. Avoid signature green on every major surface. Use pale stone color, warm wood, muted white, charcoal hardware, and furniture-like joinery; a tiny fixed moss or eucalyptus accent is allowed only in hardware or trim, never on counter, floor, wet-zone wall, or main wall. REQUIRED: all four major surfaces have zero signature green. Add no plant."
  ],
  "brass-terrazzo": [
    "COUNTER-ONLY TERRAZZO LANE. Terrazzo or chip aggregate is allowed only on the vanity counter or its short backsplash, with irregular sparse chips rather than a repeated confetti texture. EXACT BRASS FAMILY: champagne brass is allowed only on the sink faucet and its own handles. REQUIRED NON-BRASS HARDWARE: mirror frame, cabinet pulls and knobs, room-door hardware, shower or tub hardware, wall hooks, towel hardware, and every other fixture must be matte black, nickel, chrome, wood, ceramic, or painted; none may be brass, gold, bronze, or warm metal. REQUIRED SURFACES: floor = plain solid-color ceramic, porcelain, resilient, or wood-look finish with zero chips; wet-zone wall = plain solid-color tile or plaster with zero chips; main wall = ordinary warm-white, cream, or light neutral paint with zero chips.",
    "WET-ZONE-ONLY TERRAZZO LANE. Terrazzo or chip aggregate is allowed only on one shower, tub, or wainscot field. EXACT BRASS FAMILY: champagne brass is allowed only on the shower or tub controls and matching spout. REQUIRED NON-BRASS HARDWARE: sink faucet, mirror frame, cabinet pulls and knobs, room-door hardware, wall hooks, towel hardware, and every other fixture must be matte black, nickel, chrome, wood, ceramic, or painted; none may be brass, gold, bronze, or warm metal. REQUIRED SURFACES: vanity counter = solid unpatterned cream, warm white, gray, or wood with zero chips; floor = plain neutral finish with zero chips; main wall = ordinary painted plaster or drywall with zero chips.",
    "FLOOR-ONLY TERRAZZO LANE. Terrazzo or chip aggregate is allowed only on one floor field, with nonrepeating chip size, spacing, and orientation. EXACT BRASS FAMILY: champagne brass is allowed only on the vanity cabinet pulls and knobs. REQUIRED NON-BRASS HARDWARE: sink faucet, mirror frame, room-door hardware, shower or tub hardware, wall hooks, towel hardware, and every other fixture must be matte black, nickel, chrome, wood, ceramic, or painted; none may be brass, gold, bronze, or warm metal. REQUIRED SURFACES: vanity counter = solid unpatterned zero-chip finish; every wet-zone wall = plain solid-color tile or plaster with zero chips; main wall = ordinary warm-white, cream, or light neutral paint with zero chips.",
    "BRASS-ONLY OWNER-UPDATE LANE. Visible terrazzo, chip aggregate, confetti pattern, and speckled stone are forbidden everywhere. Use one muted clay, ochre, tobacco, cream, or deep neutral painted joinery decision. EXACT BRASS FAMILY: champagne brass is allowed only on the vanity cabinet pulls and knobs. REQUIRED NON-BRASS HARDWARE: sink faucet, mirror frame, room-door hardware, shower or tub hardware, wall hooks, towel hardware, and every other fixture must be matte black, nickel, chrome, wood, ceramic, or painted; none may be brass, gold, bronze, or warm metal. REQUIRED SURFACES: vanity counter, floor, wet-zone wall, and main wall all use ordinary zero-chip finishes.",
    "ALTERNATE-PALETTE JOINERY LANE. Visible terrazzo and chip aggregate are forbidden. Use furniture-like warm wood or muted painted joinery, pale solid counter material, quiet cream walls, and charcoal or nickel fixed hardware. EXACT BRASS LIMIT: choose either one single small brass cabinet knob or one narrow fixed brass trim accent, never both. Every other faucet, mirror frame, pull, knob, door part, shower or tub fixture, hook, and towel fixture must be matte black, nickel, chrome, wood, ceramic, or painted with no brass, gold, bronze, or warm metal. REQUIRED SURFACES: counter, floor, wet-zone wall, and main wall all have zero chips and no signature terrazzo pattern."
  ]
} as const;
...
export const affiliatePilotV4StyleProfiles = {
  ...
  "modern-marble": {
    palette:
      "choose one atmosphere from warm cream, cool gray, charcoal, muted green, or brown, with restrained black, chrome, brass, wood, or painted accents; the assigned fixed-surface lane alone decides whether any stone is visible, and no scene may default to white Carrara",
    architecture:
      "treat the assigned fixed-surface lane as the complete and exclusive permission map for stone or veining; every surface not explicitly permitted there must use an ordinary zero-vein finish, with properly inset basins, coherent shower glass, and compatible retained construction",
    livedIn:
      "reading glasses, a plain contact-lens case, a comb dish, a loose hair tie, or a towel used once"
  },
  "spa-greenery": {
    palette:
  ...
  "brass-terrazzo": {
    palette:
      "choose one atmosphere from warm cream, tobacco, pale gray, muted clay, desaturated green, or charcoal; the assigned fixed-surface lane alone decides whether any terrazzo or brass is visible, and no scene may repeat warm metal across multiple fixture families",
    architecture:
      "treat the assigned fixed-surface lane as the complete and exclusive permission map for terrazzo, chips, and brass; every nonassigned surface must be zero-chip and every hardware class not explicitly named must use a non-brass finish",
    livedIn:
