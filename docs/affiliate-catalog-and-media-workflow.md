...
Status: local foundation implemented; all 60 style slots are owner-approved across 59 canonical products. A rights-approved 33-asset technical pilot is complete locally. Full-scale generation still requires a separate owner decision.
...
The isolated pilot manifest records the owner's reference-rights confirmation, the three approved ASINs, six test styles, 33 deterministic local PNG keys, product-specific identity invariants, one or two required reference inputs per job, and a mandatory execution-prompt capture flag. Its `fullScaleAuthorized` value remains false.
...
The pilot used a flat chroma-green intermediate and the local reviewed chroma-key helper. All three presentation assets passed true-alpha and `1024 × 1536` canvas checks. The two generator outputs that did not initially use a 2:3 canvas were normalized without cropping, and their untouched keyed outputs were retained as evidence. Full-size visual QA passed the OXO edge treatment but rejected the curtain hooks and bamboo shelf gaps because faint green contamination remained even after the permitted one-pixel matte contraction retry. Full scale is therefore held until a non-chroma segmentation or background-removal fallback is selected and authorized.
...
The pilot made 34 generation calls for 33 final assets because one composition needed a targeted retry. At the documented GPT Image 2 portrait output rates, that is roughly `$1.39` at medium output quality or `$5.61` at high output quality before reference-image inputs, text, and any Responses API model usage. The built-in image tool did not expose an actual bill or per-call model/quality selection, so these values are planning references rather than measured pilot spend.

Recommended sequence:

1. Completed: 3 approved products × 1 presentation + 2 styles × 5 views = 33-image technical pilot.
2. Completed: review identity, reference rights, segmentation, style fidelity, prompts, cost, and QA agreement.
