...
const PHYSICAL_PHOTO_CONTRACT = [
  ...
  "Daily-use invariant: show at least three independent nondecorative signs of occupancy or age, but the assigned Human trace supplies every movable object and must be counted literally. After the assigned trace, every additional sign must be fixed evidence such as scuffed paint, worn caulk, a repaired edge, high-touch sheen, a water mark on a fixed surface, tarnished fixed hardware, or a mismatched retained finish. Do not invent extra fabric, toiletries, toys, cups, brushes, mats, open storage, or loose objects to reach three signs.",
  "Decor-limit invariant: never build a coordinated plant-art-rug vignette. Add zero optional decorative objects or movable style cues; express style only through the assigned fixed-surface lane and ordinary construction.",
  "Physical invariant: use one buildable North American bathroom. Anything visible has support and clearance; any visible sink, tub, shower, door, cabinet, glass edge, plumbing fixture, outlet, switch, or reflection is complete and coherent. If a receptacle is visible, show exactly one code-safe U.S. bathroom GFCI with coherent test/reset geometry; a standard duplex, stacked receptacles, duplicated outlets, and ambiguous wall plates are hard rejections. Omit electrical devices rather than crop or invent them.",
...
function buildStyledPrompt(
  ...
  const compositionIntent =
    selection.productRole === "countertop-dispenser" && slot === 1
      ...
      : selection.productRole === "countertop-dispenser" && slot === 4
      ? "Broad tight-room bathroom view that uses the 0.5x lens to reveal an ordinary room, never a counter close-up. Show at least two coherent non-vanity room zones such as a complete doorway plus floor circulation, toilet plus tub or shower, or wet zone plus retained period detail. Keep the vanity at no more than the lower two-thirds of the frame and prevent its counter from filling the image. Put the dispenser in an outer image third, never at the visual center, and keep it about 8–14 percent of frame height. Keep the exact fully visible wall-hook towel and its darker damp patch inside the frame, and keep the exact unsupported soap bar on the fixed vanity counter; separate both traces from the dispenser so they read as recent use rather than styling."
