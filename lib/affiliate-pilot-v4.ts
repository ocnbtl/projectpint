...
function buildStyledPrompt(
  ...
  const qaFocus = [
    ...
    "ordinary camera-roll iPhone capture rather than interior photography",
    "lived-in evidence, material-specific imperfections, physically supported objects, coherent reflections",
    "materially distinct room, camera, lighting, budget, palette, and activity within the five-scene set"
  ].join("; ");
  return {
    prompt: [
      "Use case: photorealistic-natural.",
      "Asset type: raw homeowner iPhone camera-roll photograph for private owner curation; not finished editorial, advertising, real-estate, or catalog imagery.",
      `Scene identity: ${sceneId}; corpus diversity seed: ${diversity.corpusSeed}.`,
      "Primary request: create one ordinary, unedited iPhone snapshot from a real person's camera roll in a maintained but actively used bathroom. It should look like the homeowner quickly photographed the room for a friend after a small update, not like anyone prepared the room for a shoot. At normal viewing size it must be credible as a real phone photo before it is appealing. The bathroom is the subject; the exact featured product is useful, incidental, off-center, visible, and identifiable.",
      "Camera-roll test: reject any result that reads as a render, mockup, AI home, professional interior photograph, real-estate listing, catalog set, or social-media styling. Do not beautify surfaces into uniform perfection and do not arrange daily-use objects into a vignette.",
      ...
      `Room history and budget: ${diversity.roomArchetype}; ${diversity.budget}. The room has been used since installation and was not reset for this photograph.`,
      `Camera authenticity: ${diversity.camera}; ${diversity.lighting}. Reproduce a default iPhone HEIC/JPEG look: ordinary deep focus, modest computational sharpening and local auto-HDR, slight edge distortion, imperfect leveling, fine luminance and chroma noise in shadows, mixed white balance when lights differ, at least one partially clipped highlight or blocked shadow, and no RAW processing, Lightroom grade, flash balancing, tripod precision, portrait-mode blur, or architectural correction.`,
      `Everyday evidence: ${diversity.occupancy}. These clues are functional, uncoordinated, and left where a person used them; never align, color-match, center, or style them. Do not add a plant, vase, branch, framed art, candle, tray, folded-towel stack, woven basket, slippers, robe, or coordinated spa vignette unless the concrete scene explicitly requires that exact object.`,
      `Material emphasis: ${diversity.material}.`,
      "Surface forensic realism: painted surfaces show fine roller or brush stipple, slightly uneven cut-in edges, isolated touch-up sheen, and sparse high-touch scuffs instead of spray-perfect color. Wood and bamboo are assembled from physically separate pieces: grain direction follows each board and joint, growth rings and color change between pieces, end grain appears only at cut ends, pores do not loop, and no grain motif repeats. Bamboo may show faint water spotting and nonuniform edge darkening without looking damaged. Stone has sparse unique veins with slab continuity; tile has small installation and grout-tone variation; fabric has unrelated folds and compressed fibers. Never use fractal grain, tiled veins, cloned texture islands, uniform roughness noise, plastic-perfect finish, or blanket gloss.",
      ...
      "Lived-in range: maintained and sanitary, but not pristine, empty, luxury-perfect, damaged, or decrepit. Include ordinary use, accumulated minor inconsistencies, and imperfect cleaning while keeping the room pleasant and functional.",
