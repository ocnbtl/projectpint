...
function buildTextileRoomPlatePrompt(
  ...
  const installationZoneContract =
    slot === 4
      ? "Installation zone: show the room from inside the open dry tub, with one plausible tub rim and shower-interior edge. Crop the future curtain plane below its unseen suspension line so the complete rod, mounts, hooks, reinforced openings, top edge, lintel, soffit, and upper mounting wall remain outside frame wherever the body will enter. The future partly open curtain occupies a far-side foreground shower-boundary plane rather than a blank wall. No rod fragment or mounting hardware may appear."
  ...
  const insertionContract =
    slot === 4
      ? "Reverse-view occlusion-plane gate: reserve one far-side foreground curtain plane approximately 18-35 percent of frame width, running from the top image border already below suspension to behind the tub rim. Because the camera is inside the tub, uninterrupted fixed wall, floor, tile, baseboard, or tub pixels may be visible behind this empty plane before the curtain is added; the plane does not need to be a blank backdrop. No movable human trace, grooming object, toilet-paper roll, towel, door edge, cabinet, basket, plant, art, plumbing control, fixture projecting into the curtain path, electrical device, reflection, liner, hook, or rod may occupy the plane. Put every named trace on the opposite side. A later provider-native edit may naturally occlude only fixed background pixels inside the curtain silhouette and must preserve every unoccluded room pixel."
  ...
  const plateLockPurpose =
    slot === 4
      ? "Plate-lock purpose: a later provider-native edit will add only the reviewed reverse curtain body inside the occlusion-safe foreground shower-boundary plane while the complete header remains outside frame. Natural occlusion of fixed background pixels inside the product silhouette is expected; no movable object or count-sensitive fixture may need removal, and every pixel outside the product silhouette must remain unchanged. Do not create a product-photography backdrop or hero-light the future plane."
...
function buildTextileRoomPlateCorrectionPrompt(
  ...
  const insertionResult =
    slot === 4
      ? "Reverse-view result: preserve one far-side foreground curtain plane from the top image border already below suspension to behind the tub rim. Fixed wall, floor, tile, baseboard, or tub pixels may remain visible behind that plane, but no movable trace, grooming object, toilet-paper roll, towel, door edge, cabinet, basket, plant, art, plumbing control, electrical device, reflection, liner, hook, rod, or count-sensitive fixture may occupy it. A later curtain may occlude only fixed background pixels inside its silhouette."
...
function buildStyledPrompt(
  ...
  const roomPlateCorrectionPrompt = textileRole
    ? buildTextileRoomPlateCorrectionPrompt(product, sceneId, slot)
    : null;
  ...
  const inputImageRoles = fullHeaderTextile
    ...
    : hiddenHeaderTextile
      ? slot === 4
        ? "Input image roles: Image 1 is the reviewed one-use dry-tub reverse-view room plate and the locked photographic base; its foreground curtain plane has already passed the scene-aware occlusion-safety and below-suspension gates. Preserve its camera position, framing, architecture, plumbing, light direction, exposure, objects, wear, labels, and every pixel outside the future curtain silhouette. Image 2 is the reviewed one-use scene-specific body-and-hem support generated and accepted for this job; it governs the fresh reverse body silhouette, ordinary side seam, weighted hem, and relaxed gravity state. Its body must remain one mostly broad cloth face with at most one full-height trough and one asymmetric displacement that fades into the face by the lower third. Never convert it into two or more adjacent uninterrupted channels, equal tubes, parallel stripes, rigid planes, a large diagonal trough, U-shaped scoop, swag, loop, folded-over mass, unsupported pinch, or a free hem ending at or above the tub rim. Image 3 is the reviewed exact-product material/detail crop and governs only weave behavior, color, thickness, and surface roughness; it contains no reusable product silhouette or header. Use a provider-native edit to add only the partly open reverse curtain body into the far-side foreground shower-boundary plane, entering through the top frame already below its hidden suspension line and continuing naturally behind the tub rim. Natural occlusion of uninterrupted fixed wall, floor, tile, baseboard, or tub pixels inside the curtain silhouette is physically expected and is not room restaging. Do not remove or move any human trace, movable object, door edge, cabinet, plumbing control, fixture, electrical device, or pixel outside the product silhouette. The rod, mounts, hooks, openings, top edge, full lintel, and soffit remain outside frame. Do not redesign, restage, widen, clean, relight, or recrop the room."
        : "Input image roles: Image 1 is the reviewed one-use hidden-header room plate and the locked photographic base; its shower crop has already passed the below-suspension-line feasibility gate. Preserve its camera position, framing, architecture, plumbing, light direction, exposure, objects, wear, labels, and human irregularity. Image 2 is the reviewed one-use scene-specific body-and-hem support generated and accepted for this job; it governs the fresh body silhouette, ordinary side seam, weighted hem, and relaxed gravity state. Its body must remain one mostly broad cloth face with at most one full-height trough and one asymmetric displacement that fades into the face by the lower third. Never convert it into two or more adjacent uninterrupted channels, equal tubes, parallel stripes, rigid planes, a large diagonal trough, U-shaped scoop, swag, loop, folded-over mass, unsupported pinch, or a free hem ending at or above the tub rim. Image 3 is the reviewed exact-product material/detail crop and governs only weave or print behavior, color, thickness, and surface roughness; it contains no reusable product silhouette or header. Use a provider-native edit to add only the partly open curtain body inside the empty lower shower opening, entering through the top frame already below its hidden suspension line and continuing naturally below the tub rim toward the shower interior or floor. The rod, mounts, hooks, openings, top edge, full lintel, and soffit remain outside frame. Do not redesign, restage, widen, clean, relight, or recrop the room."
